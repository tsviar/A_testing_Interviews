...
// find the Maximal positive int that has a negative twin
// [3, 2, -2, 5, -3] => -3,3  => 3
// [1, 1, 2, -1, 2, -1] => 1

const findTheMaximalPosNegPair = InArrPairs => {
  const lenArr = InArrPairs.length | 0;
  let theArr = InArrPairs;

  theArr.sort((a, b) => a - b); // asc
  // [3, 2, -2, 5, -3] => [-3, -2, 2, 3, 5]
  console.log("findTheMaximalPosNegPair theArr", theArr);

  // minimal and maximal are both negative or positive
  //  half the scale is missing
  if (theArr[0] * theArr[lenArr - 1] > 0) {
    return 0;
  }
  ...
  while (i > 0 && theArr[i] > 0 && matchingNegIdx < 0) {
    matchingNegIdx = theArr.findIndex(a => a === -1 * theArr[i]);
    console.log(
      `findTheMaximalPosNegPair theArr[i] ${
        theArr[i]
      } matchingNegIdx[i] ${matchingNegIdx}`
    );
    i--;
  }

  if (matchingNegIdx >= 0 && theArr[matchingNegIdx] < 0) {
    max = -1 * theArr[matchingNegIdx];
  }

  return max;
};

// const sampleA = [3, 2, -2, 5, -3];
const sampleA = [1, 1, 2, -1, 2, -1];

var t0 = performance.now();

let maxPos = findTheMaximalPosNegPair(sampleA);

var t1 = performance.now();

console.log("findTheMaximalPosNegPair maxPos", maxPos);
console.log(
  "Call to findTheMaximalPosNegPair took " + (t1 - t0) + " milliseconds."
);

// find if the Maximal negative int has a positive twin
// [3, 2, -2, 5, -3] => -3,3  => ok
// [1, 1, 2, -1, 2, -1] => no

const findIfTheMaximalPosHasNegPair = InArrPairs => {
  const lenArr = InArrPairs.length | 0;
  let theArr = InArrPairs,
    foundMax = 0;
  ...
  theArr.sort((a, b) => a - b); // asc
  // [3, 2, -2, 5, -3] => [-3, -2,  2, 3, 5]
  console.log("findIfTheMaximalPosHasNegPair theArr", theArr);
  ...
  console.log(
    "findIfTheMaximalPosHasNegPair theArr[lenArr-1]",
    theArr[lenArr - 1]
  );
  const matchingPosIdx = theArr.findIndex(a => a === -1 * theArr[lenArr - 1]);

  if (matchingPosIdx > 0) {
    foundMax = theArr[matchingPosIdx];
  }

  return foundMax;
};

const sampleA2 = [3, 2, -2, 5, -3];
// const sampleA2 = [1, 1, 2, -1, 2, -1];

var t0 = performance.now();

let max2 = findIfTheMaximalPosHasNegPair(sampleA2);

var t1 = performance.now();

console.log("findIfTheMaximalPosHasNegPair max", max2);
console.log(
  "Call to findIfTheMaximalPosHasNegPair took " + (t1 - t0) + " milliseconds."
);

// thumb law for averages
// lowest is always tow or three numbers,
//4 numbers and up, the average rises

const MinAvgTwoSlice3 = A => {
